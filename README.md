...
> **📢 開発一時停止のお知らせ（v2.1.0）**
>
> 本プロジェクトは v2.1.0 で開発を一時停止しています。
> 今後は CodeLodis（デスクトップ版統合基盤）への移行を計画しており、本リポジトリはリファレンスとして保持されます。
> 既存機能は引き続き使用可能です。
...
| ドキュメント | 説明 |
|-------------|------|
| [ユーザーガイド](docs/user-guide.md) | 詳細な取扱説明書（設定・カスタマイズ・トラブルシューティング） |
| [バージョンアップ手順](docs/user-guide.md#6-5-バージョンアップ手順) | 拡張機能のアップデート方法と注意点 |
| [アンインストール手順書](docs/アンインストール手順書.md) | アンインストール時のリソースクリーンアップ手順 |
