...
const { executeListTasks, compareTaskIds } = await import("../../services/task-manager.js");
...
describe("executeListTasks - reviewed/starred フィルタ", () => {
    describe("reviewed フィルタ", () => {
        it("reviewed=true → reviewed済みタスクのみ返す（A, B）", async () => {
            ...
            expect(result.tasks.map((t) => t.id)).toEqual(expect.arrayContaining(["048", "048-1"]));
        ...
        it("reviewed=false → 未reviewedタスクのみ返す（C, D, E）", async () => {
            ...
            expect(result.tasks.map((t) => t.id)).toEqual(expect.arrayContaining(["048-2", "048-10", "047"]));
            result.tasks.forEach((t) => {
    ...
    describe("starred フィルタ", () => {
        it("starred=true → starred済みタスクのみ返す（A, C）", async () => {
            ...
            expect(result.tasks).toHaveLength(2);
            expect(result.tasks.map((t) => t.id)).toEqual(expect.arrayContaining(["048", "048-2"]));
            result.tasks.forEach((t) => {
        ...
        it("starred=false → 未starredタスクのみ返す（B, D, E）", async () => {
            ...
            expect(result.tasks).toHaveLength(3);
            expect(result.tasks.map((t) => t.id)).toEqual(expect.arrayContaining(["048-1", "048-10", "047"]));
            result.tasks.forEach((t) => {
    ...
    describe("複合フィルタ（AND条件）", () => {
        ...
        it("reviewed=false + starred=false → D, E", async () => {
            ...
            expect(result.tasks).toHaveLength(2);
            expect(result.tasks.map((t) => t.id)).toEqual(expect.arrayContaining(["048-10", "047"]));
        });
    });
