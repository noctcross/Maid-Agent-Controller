...
describe("executeSideEffects", () => {
  describe("syncMaidYaml - maid yaml 自動同期", () => {
    it("assignees 変更時に maid yaml が更新される", async () => {
      ...
      const prevAssignees: Array<{ agentId: string; role: string | null; subTaskId: string | null }> = [];
      ...
      const writtenData = mockedWriteYamlFile.mock.calls[0][1] as Record<string, unknown>;
  ...
  describe("archiveReport - レポートアーカイブ", () => {
    it("status が completed に変更された場合、レポートがコピーされる", async () => {
      ...
      expect(mockedCopyFile).toHaveBeenCalledWith(
        "/project/.maid-agent/system/data/reports/current_emma.md",
    ...
    it("params.agentId を優先して使用する", async () => {
      ...
      expect(mockedCopyFile).toHaveBeenCalledWith(
        "/project/.maid-agent/system/data/reports/current_sophia.md",
        expect.stringContaining("sophia")
      );
    });
