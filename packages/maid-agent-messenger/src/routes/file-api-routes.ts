 ...
 * ファイル/ディレクトリが表示可能か判定
 */
function isVisibleItem(name: string): boolean {
...
function isViewableFile(filename: string): boolean {
  ...
  if (!ext) {
    return ["README", "LICENSE", "CHANGELOG", "CLAUDE"].some((name) =>
...
async function validatePath(
  ...
  if (
    !resolvedPath.startsWith(normalizedProjectPath + path.sep) &&
...
router.get("/api/files", async (req: Request, res: Response) => {
  try {
    ...
    for (const entry of entries) {
      if (!isVisibleItem(entry.name)) continue;
