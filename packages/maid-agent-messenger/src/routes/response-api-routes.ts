 ...
 * GET /api/responses - エージェントの応答履歴取得
 */
router.get("/api/responses", async (req: Request, res: Response) => {
  try {
    ...
    let maidConfig: MaidConfig;
    try {
      const configContent = await fs.readFile(maidConfigPath, "utf-8");
      maidConfig = yaml.parse(configContent) as MaidConfig;
    } catch {
 ...
 */
router.get("/api/agents/sessions", async (req: Request, res: Response) => {
  try {
    const projectPath = req.query.project as string;
    ...
    for (const file of files) {
      if (file.endsWith(".yaml") && file !== ".gitkeep") {
        ...
        try {
          ...
          const config = yaml.parse(content) as MaidConfig;
