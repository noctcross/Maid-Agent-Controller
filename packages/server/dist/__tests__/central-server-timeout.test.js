...
describe("central-server - Express timeout settings", () => {
    it("keepAliveTimeout と headersTimeout が設定ファイルの値で正しく定義される", () => {
        ...
        expect(config.http_headers_timeout).toBeGreaterThan(config.http_keepalive_timeout);
    });
    it("デフォルト値がプロキシの60秒タイムアウトより長い", async () => {
        ...
        expect(fullConfig.keepalive.http_headers_timeout).toBeGreaterThan(fullConfig.keepalive.http_keepalive_timeout);
