...
const mockExecuteGetReport = jest.fn().mockResolvedValue({
    success: true,
    reports: [],
});
// V2用モック
const mockGenerateV2DashboardData = jest.fn();
...
jest.unstable_mockModule("../../utils/path-helpers.js", () => ({
    getQueueMaidPath: (p) => `${p}/.maid-agent/system/data/maid`,
}));
jest.unstable_mockModule("../../services/project-registry.js", () => ({
    recordProjectAccess: jest.fn().mockResolvedValue(undefined),
}));
// --- テストデータ ---
...
// --- ビュースタブ（V2対応） ---
const stubGenerateDashboardHtml = jest.fn()
...
const stubGenerateTaskHtml = jest.fn()
    .mockReturnValue("<div>stub tasks</div>");
const stubComposeMasterWaitingHtml = jest.fn()
    .mockReturnValue("<div>stub waiting</div>");
// --- テスト用app構築 ---
const app = express();
app.use(express.json());
app.use(createDashboardRoutes({
    generateDashboardHtml: stubGenerateDashboardHtml,
    generateTaskHtml: stubGenerateTaskHtml,
    composeMasterWaitingHtml: stubComposeMasterWaitingHtml,
}));
/** モック設定ヘルパー */
...
describe("TC-E1-1: Goalsツリー表示", () => {
    ...
    it("generateDashboardDataが呼び出される", async () => {
        await supertest(app)
            .get("/dashboard?version=v2")
            .set("X-Maid-Project-Path", TEST_PROJECT_PATH);
        expect(mockGenerateV2DashboardData).toHaveBeenCalledWith(TEST_PROJECT_PATH, expect.any(Object));
    });
});
// ===========================================
// TC-E1-2: ページネーション
...
describe("TC-E1-2: ページネーション", () => {
    ...
    it("V2データAPIでlimitパラメータが適用される", async () => {
        await supertest(app)
            ...
            .set("X-Maid-Project-Path", TEST_PROJECT_PATH);
        expect(mockGenerateV2DashboardData).toHaveBeenCalledWith(TEST_PROJECT_PATH, expect.objectContaining({ limit: 20 }));
    });
    it("V2データAPIでoffsetパラメータが適用される", async () => {
        await supertest(app)
            .get("/dashboard/goals?offset=10")
            .set("X-Maid-Project-Path", TEST_PROJECT_PATH);
        expect(mockGenerateV2DashboardData).toHaveBeenCalledWith(TEST_PROJECT_PATH, expect.objectContaining({ offset: 10 }));
    });
});
// ===========================================
// TC-E1-3: フィルタ切り替え
...
describe("TC-E1-3: フィルタ切り替え", () => {
    it("status=openでstatusFilter=openが渡される", async () => {
        await supertest(app)
            ...
            .set("X-Maid-Project-Path", TEST_PROJECT_PATH);
        expect(mockGenerateV2DashboardData).toHaveBeenCalledWith(TEST_PROJECT_PATH, expect.objectContaining({ statusFilter: "open" }));
    });
    it("status=closedでstatusFilter=closedが渡される", async () => {
        await supertest(app)
            .get("/dashboard/goals?status=closed")
            .set("X-Maid-Project-Path", TEST_PROJECT_PATH);
        expect(mockGenerateV2DashboardData).toHaveBeenCalledWith(TEST_PROJECT_PATH, expect.objectContaining({ statusFilter: "closed" }));
    });
    it("status=allでstatusFilter=allが渡される", async () => {
        await supertest(app)
            .get("/dashboard/goals?status=all")
            .set("X-Maid-Project-Path", TEST_PROJECT_PATH);
        expect(mockGenerateV2DashboardData).toHaveBeenCalledWith(TEST_PROJECT_PATH, expect.objectContaining({ statusFilter: "all" }));
    });
