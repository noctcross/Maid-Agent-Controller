...
const HISTORY_LOG_PATH = path.join(TEST_PROJECT_PATH, ".maid-agent/system/data/notifications/history.log");
// ルーターをダイナミックインポート
...
function getPostHandler() {
    const stack = notificationApiRoutes.stack;
    const postRoute = stack.find((layer) => layer.route?.path === "/api/notifications" && layer.route?.methods?.post);
    return postRoute?.route?.stack[0]?.handle;
}
// ルーターからGETハンドラを取得
function getGetHandler() {
    const stack = notificationApiRoutes.stack;
    const getRoute = stack.find((layer) => layer.route?.path === "/api/notifications" && layer.route?.methods?.get);
...
describe("POST /api/notifications", () => {
    ...
    it("toパラメータが未指定の場合は400を返す", async () => {
        const { req, res } = createMockReqRes({ project: TEST_PROJECT_PATH }, { message: "test" });
        await postHandler(req, res);
        expect(res.status).toHaveBeenCalledWith(400);
    ...
    it("messageパラメータが未指定の場合は400を返す", async () => {
        const { req, res } = createMockReqRes({ project: TEST_PROJECT_PATH }, { to: "chief" });
        await postHandler(req, res);
        expect(res.status).toHaveBeenCalledWith(400);
        expect(res.json).toHaveBeenCalledWith({
            error: "Missing 'to' or 'message' in request body",
    ...
    it("無効なターゲットの場合は400を返す", async () => {
        const { req, res } = createMockReqRes({ project: TEST_PROJECT_PATH }, { to: "invalid_target", message: "test" });
        await postHandler(req, res);
        expect(res.status).toHaveBeenCalledWith(400);
    ...
    it("有効なリクエストでhistory.logに追記される", async () => {
        const { req, res } = createMockReqRes({ project: TEST_PROJECT_PATH }, { to: "chief", message: "テストメッセージ" });
        await postHandler(req, res);
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({
            success: true,
    ...
    it("複数のターゲットに送信できる", async () => {
        ...
        for (const target of validTargets) {
            const { req, res } = createMockReqRes({ project: TEST_PROJECT_PATH }, { to: target, message: `Message to ${target}` });
            await postHandler(req, res);
            expect(res.json).toHaveBeenCalledWith(expect.objectContaining({
                success: true,
                notification: expect.objectContaining({
    ...
    it("レスポンスのnotificationにidが含まれる", async () => {
        const { req, res } = createMockReqRes({ project: TEST_PROJECT_PATH }, { to: "chief", message: "test" });
        await postHandler(req, res);
...
`;
        ...
        const jsonCall = res.json.mock.calls[0][0];
        expect(jsonCall.notifications).toHaveLength(2);
        expect(jsonCall.notifications.every((n) => n.from === "emma" || n.to === "emma")).toBe(true);
    });
