...
const mockExecuteGetTask = jest.fn();
const mockExecuteUpdateTask = jest.fn();
const mockExecuteGetReport = jest.fn();
const mockArchiveReport = jest.fn();
const mockMigrate = jest.fn();
const mockCheckMigrationStatus = jest.fn();
const mockExecuteGetTeamStatus = jest.fn();
const mockGenerateDashboardData = jest.fn();
...
describe("GET /api/tasks", () => {
    ...
    it("statusフィルタを正しくパースする", async () => {
        ...
        expect(mockExecuteListTasks).toHaveBeenCalledWith(TEST_PROJECT_PATH, expect.objectContaining({ status: ["pending", "working"] }));
    });
    it("assigneeフィルタを正しくパースする", async () => {
        mockExecuteListTasks.mockResolvedValue(createMockListResponse());
        await supertest(app).get("/api/tasks?assignee=emma").expect(200);
        expect(mockExecuteListTasks).toHaveBeenCalledWith(TEST_PROJECT_PATH, expect.objectContaining({ assignee: "emma" }));
    });
    it("parentIdフィルタを正しくパースする（値あり）", async () => {
        mockExecuteListTasks.mockResolvedValue(createMockListResponse());
        await supertest(app).get("/api/tasks?parentId=095").expect(200);
        expect(mockExecuteListTasks).toHaveBeenCalledWith(TEST_PROJECT_PATH, expect.objectContaining({ parentId: "095" }));
    });
    it("parentId=nullをnullとしてパースする", async () => {
        mockExecuteListTasks.mockResolvedValue(createMockListResponse());
        await supertest(app).get("/api/tasks?parentId=null").expect(200);
        expect(mockExecuteListTasks).toHaveBeenCalledWith(TEST_PROJECT_PATH, expect.objectContaining({ parentId: null }));
    });
    it("limit・offsetを数値としてパースする", async () => {
        mockExecuteListTasks.mockResolvedValue(createMockListResponse());
        await supertest(app).get("/api/tasks?limit=10&offset=5").expect(200);
        expect(mockExecuteListTasks).toHaveBeenCalledWith(TEST_PROJECT_PATH, expect.objectContaining({ limit: 10, offset: 5 }));
    });
    it("sortField・sortOrderを正しくパースする", async () => {
        mockExecuteListTasks.mockResolvedValue(createMockListResponse());
        await supertest(app).get("/api/tasks?sortField=id&sortOrder=desc").expect(200);
        expect(mockExecuteListTasks).toHaveBeenCalledWith(TEST_PROJECT_PATH, expect.objectContaining({ sortField: "id", sortOrder: "desc" }));
    });
...
describe("GET /api/tasks/:id", () => {
    it("タスク詳細を返す", async () => {
        ...
        expect(mockExecuteGetTask).toHaveBeenCalledWith(TEST_PROJECT_PATH, expect.objectContaining({ taskId: "095", includeSubtasks: false }));
    });
    it("includeSubtasks=trueを正しくパースする", async () => {
        ...
        expect(mockExecuteGetTask).toHaveBeenCalledWith(TEST_PROJECT_PATH, expect.objectContaining({ includeSubtasks: true }));
    });
...
describe("PATCH /api/tasks/:id", () => {
    it("タスクを更新する", async () => {
        ...
        expect(mockExecuteUpdateTask).toHaveBeenCalledWith(TEST_PROJECT_PATH, expect.objectContaining({ taskId: "095", status: "completed", summary: "Done" }));
    });
    it("存在しないIDに404を返す", async () => {
...
describe("GET /api/tasks/:id/report", () => {
    ...
    it("limit指定を正しくパースする", async () => {
        ...
        expect(mockExecuteGetReport).toHaveBeenCalledWith(TEST_PROJECT_PATH, expect.objectContaining({ taskId: "095", limit: 5 }));
    });
    it("存在しないIDに404を返す", async () => {
...
describe("PATCH /api/tasks/:id/review", () => {
    ...
    it("reviewed未指定時はデフォルトtrueで呼ぶ", async () => {
        ...
        expect(mockExecuteUpdateTask).toHaveBeenCalledWith(TEST_PROJECT_PATH, expect.objectContaining({ reviewed: true }));
    });
    it("存在しないIDに404を返す", async () => {
        mockExecuteUpdateTask.mockResolvedValue({ success: false });
...
describe("PATCH /api/tasks/:id/star", () => {
    ...
    it("starred未指定時はデフォルトtrueで呼ぶ", async () => {
        ...
        expect(mockExecuteUpdateTask).toHaveBeenCalledWith(TEST_PROJECT_PATH, expect.objectContaining({ starred: true }));
    });
    it("存在しないIDに404を返す", async () => {
        mockExecuteUpdateTask.mockResolvedValue({ success: false });
