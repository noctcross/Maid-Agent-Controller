...
jest.unstable_mockModule("../../utils/path-helpers.js", () => ({
    ...
    getCurrentReportsPath: (p) => `${p}/.maid-agent/system/data/reports`,
    getArchiveReportsPath: (p) => `${p}/.maid-agent/master/reports`,
}));
const { loadProjectRegistry, saveProjectRegistry, recordProjectAccess, listProjects, togglePin, toggleHide, } = await import("../../services/project-registry.js");
describe("project-registry", () => {
    ...
    describe("loadProjectRegistry", () => {
        ...
        it("既存のファイルを読み込む", async () => {
            ...
            await saveProjectRegistry(testData);
            const registry = await loadProjectRegistry();
