...
describe("DashboardWebSocketServer", () => {
    ...
    it("projectパラメータ付きで接続できる", (done) => {
        const ws = new WebSocket(`ws://localhost:${TEST_PORT}/dashboard/ws?project=/test/path`);
        ws.on("message", (data) => {
    ...
    it("異なるプロジェクトにはブロードキャストが届かない", (done) => {
        const ws = new WebSocket(`ws://localhost:${TEST_PORT}/dashboard/ws?project=/other/path`);
    ...
    describe("Escalation Notification", () => {
        ...
        it("broadcastAllEscalationで全クライアントに通知できる", (done) => {
            const ws1 = new WebSocket(`ws://localhost:${TEST_PORT}/dashboard/ws?project=/path1`);
            const ws2 = new WebSocket(`ws://localhost:${TEST_PORT}/dashboard/ws?project=/path2`);
