...
const {
  ...
  findAgentImages,
  generateAgentBackgroundSnippet,
  AGENT_IDS,
  IMAGE_EXTENSIONS,
} = await import("../../utils/agent-image.js");
...
describe("extractAgentIdFromPath", () => {
  ...
  it("task-081-alice-説明文.md（説明付き）からエージェントIDを抽出する", () => {
    const result = extractAgentIdFromPath(
      "/reports/task-081-alice-MCP接続断の継続調査 - キ.md",
    );
    expect(result).toBe("alice");
  });
  ...
  it("説明文に別のエージェント名が含まれていても先頭のIDを抽出する", () => {
    const result = extractAgentIdFromPath(
      "/reports/task-099-lily-今日のemmaの観察日記.md",
    );
    expect(result).toBe("lily");
  });
