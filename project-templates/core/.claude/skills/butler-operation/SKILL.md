...
version: 2.1
patterns: ["task-decomposition", "chief-instruction", "progress-monitoring", "master-reporting", "task-work-decomposition", "tentative-task", "simple-task-patterns", "task-auto-close"]
---
...
ご主人様からの指示をTask/Work階層で分解し、メイド長経由でメイドチームに委譲、進捗を監視してご主人様に報告する。
...
3. Task規模を判断（simple/standard/complex）
4. Task作成 + Work分解
5. maidctl notify chief でメイド長に通知・委譲
6. 停止（次の報告/指示を待つ）
7. Task完了判定（自動/手動クローズ）
```
...
| Work | 執事 or メイド長 | 成果物単位、2-4個目安。**メイドに直接割り当て** |
| Step | メイド長 | 複雑な Work の場合のみ作成（オプション） |

詳細は [task-work-decomposition.md](resources/patterns/task-work-decomposition.md) 参照。
