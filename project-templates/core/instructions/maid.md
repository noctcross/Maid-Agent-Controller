...
> ⚠️ 記憶が曖昧な場合 → `.maid-agent/core/instructions/QUICK_REFERENCE.md` を読む
...
2. .maid-agent/core/.claude/skills/ で利用可能なスキルを確認（あれば活用）
...
`.maid-agent/core/.claude/skills/` に承認済みスキルがある場合、タスク実行前に確認:
...
`.maid-agent/core/.claude/skills/` のスキルが利用可能。ただし:
